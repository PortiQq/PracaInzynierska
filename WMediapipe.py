...
print("2 - TESTY (Tworzenie zbioru testowego - Użycie poprzednich danych kalibracyjnych)")
print("3 - MENU (Użycie poprzednich danych kalibracyjnych)")
...
if mode == '1':
    ...
    current_output_file = VALIDATION_FILE  # Zbiór do weryfikacji
    write_header(current_output_file, 'validation')
    samples_per_point = VALIDATION_SAMPLES
    print(f"Wybrano TRYB WALIDACJI. Zapis do: {current_output_file}, Próbek na punkt: {samples_per_point}")
elif mode == "3":
    calibration_done = True
    try:
    ...
    print(f"Wybrano TRYB KURSORA. Zapis do: {current_output_file}")
else:
...
with mp_face_mesh.FaceMesh(
    ...
    while True:
        ...
        if result.multi_face_landmarks:
            ...
            yaw = yaw_filter(yaw, YAW_THRESHOLD)    # Filtrowanie szumu YAW przy twarzy zwróconej na wprost
            ...
            if not calibration_done:
                ...
                if start_cursor:
                    ...
                    if current_point_index < len(CALIBRATION_POINTS):   # TODO: Zmiana na osobną planszę bo póki co to ta sama co do kalibracji
                        point = CALIBRATION_POINTS[current_point_index]
                        point_x = int(point[0] * calibration_frame.shape[1])
                        point_y = int(point[1] * calibration_frame.shape[0])
            ...
            cv2.putText(debug_view, f"L Rel: {l_relative_x:.2f}, {l_relative_y:.6f}", (20, 150),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
            cv2.putText(debug_view, f"R Rel: {r_relative_x:.2f}, {r_relative_y:.6f}", (20, 170),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
            if blink_flag:
        ...
        if key == ord(' '):
            if not calibration_done:
                ...
                continue
# TODO: funkcja resetująca zbieranie punktów?
# TODO: funkcja zmieniająca plik do zapisu i wywołująca print oraz write_header?
        elif key == ord('t'):
            if calibration_done:
                validation_flag = True
                start_cursor = False
